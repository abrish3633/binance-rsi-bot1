...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            # === FORCE ONE-WAY MODE ON EVERY RESTART (Manual or Daily) ===
            try:
                response = bot_state.client.send_signed_request(
                    "POST", 
                    "/fapi/v1/positionSide/dual", 
                    {"dualSidePosition": "false"}
                )
                log("✅ Successfully forced ONE-WAY mode on restart", args.telegram_token, args.chat_id)
            except Exception as e:
                error_msg = str(e).lower()
                if any(keyword in error_msg for keyword in ["position", "cannot change", "has open position", "already"]):
                    log("ℹ️ One-Way mode is already set or cannot be changed (positions may exist)", 
                        args.telegram_token, args.chat_id)
                else:
